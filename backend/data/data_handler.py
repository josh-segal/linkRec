...
class DataHandler:
    ...
    def load_data(self, sample_fraction: float = 1.0, random_state: Optional[int] = None) -> None:
        ...
        self.news_df = pd.read_csv(f'{self.data_path}/MINDsmall_train/news.tsv', 
                                 sep='\t', header=None, names=col_news)
        
        # Load behaviors data
        col_behaviors = ['ImpressionID', 'UserID', 'Time', 'History', 'Impressions']
        self.behaviors_df = pd.read_csv(f'{self.data_path}/MINDsmall_train/behaviors.tsv', 
        ...
        self.feature_dim = 4 + len(self.news_features.columns)  # 4 context features + one-hot features
    ...
    def _encode_categorical_features(self, news_df: pd.DataFrame) -> pd.DataFrame:
        ...
        cat_clusters = cat_kmeans.fit_predict(cat_embeddings)
        subcat_clusters = subcat_kmeans.fit_predict(subcat_embeddings)
        
        # Create mappings and encode
    ...
    def get_random_context(self) -> Dict:
        ...
        Returns:
            Dict containing context features and available articles
        """
        interaction = np.random.choice(self.processed_data)
        return {
            'context': interaction['context'],
            'articles': interaction['news_features'],
            'true_rewards': interaction['rewards']  # For evaluation purposes
        }
